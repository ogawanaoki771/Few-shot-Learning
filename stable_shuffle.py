...
if __name__ == "__main__":
    ...
    for sample in X_train:
        ...
        X_train_shuffled.append(arr)
        
    # テストデータの各サンプルの点集合をランダムシャッフル
    X_test_shuffled = []
    for sample in X_test:
        arr = np.array(sample, dtype=float)
        np.random.shuffle(arr)
        X_test_shuffled.append(arr)
    print(f" |- Train Family : {len(X_train_shuffled)} shapes (Closed: {sum(y_train==0)}, Open: {sum(y_train==1)})")
    print(f" |- Test Family  : {len(X_test_shuffled)} shapes (Closed: {sum(y_test==0)}, Open: {sum(y_test==1)})\n")
    
    eco = StochasticMetaEcosystem(init_count=30)
    eco_before = copy.deepcopy(eco)
    
    print("Ecosystem Learning Progress on Train Family...")
    for epoch in range(2):
        for idx in range(len(X_train_shuffled)):
            eco.cycle(X_train_shuffled[idx], label=y_train[idx], steps=28, is_training=True)
            
    print("Evaluating on Unseen Test Family with NON-DESTRUCTIVE Test-Time Adaptation...")
    metrics = evaluate_concept_learning(eco, X_train_shuffled, y_train, X_test_shuffled, y_test)
    print("\n" + "="*60)
    ...
    plot_bp015_research_dashboard(
        eco=eco, 
        sample_coords=X_test_shuffled[0], 
