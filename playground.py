...
class LineAdaptiveCell:
    ...
    def access(
        ...
        # =====================================================
        # 🔥 3. Dynamic anchor movement
        # =====================================================

        # -----------------------------------------------------
        # A. Attraction to input
        # -----------------------------------------------------
    ...
    def prune_links(self):
        ...
        for c in self.pool:
            ...
            c.links = {
                ...
                in c.links.items()
                if weight
                >= self.min_link_weight
            }

            # -------------------------------------------------
            # Keep only strongest links
            # -------------------------------------------------
...
def evaluate_concept_learning(
    ...
    print(
        "[Feature Extraction] Testing with TTA..."
    )
...
def print_anchor_statistics(
    ...
    print(
        f"Number of Cells       : "
        f"{len(total_distances)}"
    )

    print(
        f"Mean Total Path       : "
        f"{np.mean(total_distances):.3f}"
    )

    print(
        f"Mean Displacement     : "
        f"{np.mean(displacement_distances):.3f}"
    )

    print(
        f"Max Total Path        : "
        f"{np.max(total_distances):.3f}"
    )

    print(
        f"Max Displacement      : "
        f"{np.max(displacement_distances):.3f}"
    )
...
def plot_bp015_research_dashboard(
    ...
    # ここでは access() がアンカーを動かす。
    # 可視化では、現在位置に対する応答を取得する。
    acts = [
        c.access(
            sample_coords,
            id_map
    ...
    plt.suptitle(
        ...
        "(Dynamic Anchors + Repulsion + Exploration + Sparse Links)",
        fontsize=14,
        fontweight="bold",
        y=0.98
    )
...
# =========================================================
# 📊 9. Pixel Count vs Logistic Output
# =========================================================
...
if __name__ == "__main__":
    ...
    print(
        "Attraction + Repulsion + Exploration + Sparse Links"
    )
    ...
    X_train, y_train = generate_diverse_shapes(
        num_samples=12,
        img_size=IMG_SIZE,
        seed=101,
        family_type="train"
    ...
    X_test, y_test = generate_diverse_shapes(
        num_samples=34,
        img_size=IMG_SIZE,
        seed=999,
        family_type="test"
    ...
    print(
        "\n[6] Evaluating on unseen test family..."
    )

    metrics = evaluate_concept_learning(
        eco,
        X_train_shuffled,
        y_train,
        X_test_shuffled,
    ...
    print(
        "\n[8] Generating Research Dashboard..."
    )
    ...
    # =====================================================
    # Dynamic anchor trajectory
    # =====================================================

    print(
        "\n[9] Generating Anchor Trajectory..."
    )
